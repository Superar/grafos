...
def dijkstra(grafo, r):
    ...
    while not fila_q.empty():
        ...
        for v in nx.all_neighbors(grafo, u):
            if v in lista_fila and grafo.node[v]['lam'] > (grafo.edge[u][v]['weight'] + grafo.node[u]['lam']):
...
def main():
    ...
    print('Gerando árvores de Dijkstra...')
    raizes = [1, 16, 3]
    g_out = dijkstra(g_in, raizes)
